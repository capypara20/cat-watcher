...
| Phase 9（コピー） | 正常コピー、上書きスキップ（WARN）、preserve_structure、リトライ |
| Phase 10（移動） | 正常移動、異ボリュームフォールバック、プレースホルダ更新 |
...
| **3** | 🔲 未着手 | 設定読み込み | `config.rs`（構造体 + デシリアライズ） | `serde`, `toml`, derive マクロ |
| **4** | 🔲 未着手 | CLI | `main.rs`（clap 引数パース） | `clap` derive API |
| **5** | 🔲 未着手 | バリデーション | `config.rs`（バリデーション関数群） | `Path`, `canonicalize`, グラフ探索 |
| **6** | 🔲 未着手 | プレースホルダ | `placeholder.rs` | 文字列パーサ, `chrono` (or 標準 `time`) |
...
- [ ] 全バリデーション項目が実装されている
- [ ] 不正な設定ファイルを食わせるとエラーメッセージ付きで exit 1 になる
- [ ] `--validate` で正常設定ファイルを検証すると exit 0 になる
- [ ] ユニットテストが全件パスする
...
5. エラー時はリトライ（retry_count / retry_interval_ms）
6. 成功後、context.destination を更新
```
...
4. rename 失敗かつ cross-device → copy + 元ファイル削除 にフォールバック
5. 成功後、context の FullName 等を移動先に更新
6. context.destination を更新
```
